...
<script>
import { initialLocationsData, initialNextId } from "@/data/db.js"; // Import the data

export default {
  name: "UrbexCrudPage",
  data() {
    return {
      // Use a deep copy of the imported data so modifications don't affect the original module
      locations: JSON.parse(JSON.stringify(initialLocationsData)),
      formData: {
        id: null,
        name: "",
        date: "",
      ...
      isEditing: false,
      nextId: initialNextId, // Initialize nextId from the imported value
    };
  },
  methods: {
    generateId() {
    ...
    startEdit(location) {
      this.isEditing = true;
      this.formData = { ...location }; // Create a copy to edit
    },
    updateEntry() {
      if (
        !this.formData.name ||
